...
export const MarketBookCard: FC = () => {
  ...
  if (loading || !engine || !config || !params) {
    return (
      <div className="p-4">
        <p className="text-sm text-[#8B95B0]">{loading ? "Loading…" : "—"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      {/* Price ladder */}
      <div className="mb-4 grid grid-cols-3 gap-px overflow-hidden rounded-lg bg-white/[0.06]">
        <div className="bg-[#080a0f] p-3 text-center">
          <p className="text-[9px] uppercase tracking-wider text-[#8B95B0]">Bid</p>
          <p className="data-cell text-sm font-medium text-[#00e68a]">${(bestBid / 1_000_000).toFixed(6)}</p>
        </div>
        <div className="bg-[#080a0f] p-3 text-center">
          <p className="text-[9px] uppercase tracking-wider text-[#8B95B0]">Oracle</p>
          <p className="data-cell text-sm font-medium text-[#F0F4FF]">{formatUsd(oraclePrice)}</p>
        </div>
        <div className="bg-[#080a0f] p-3 text-center">
          <p className="text-[9px] uppercase tracking-wider text-[#8B95B0]">Ask</p>
      ...
      {lps.length > 0 && (
        <div>
          <div className="mb-1 flex text-[9px] uppercase tracking-wider text-[#3D4563]">
              ...
              <div key={idx} className="flex items-center border-t border-white/[0.06] py-1.5 text-[11px]">
                ...
                <span className="w-20 pl-3">
                  <div className="h-1 rounded-full bg-white/[0.06]">
                    <div className="h-1 rounded-full bg-[#00FFB2]/50" style={{ width: `${pct}%` }} />
                  </div>
                </span>
              </div>
            );
